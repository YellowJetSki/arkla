...
export default function SpellDiscovery({ className, onAddSpell, allowAdd = true, maxSpellLevel = 9 }) {
  ...
  useEffect(() => {
    const initSpells = async () => {
      ...
      let homebrewStubs = [];
      try {
        const snap = await getDocs(collection(db, 'homebrew_spells'));
      ...
      } catch (e) { console.error("Error fetching homebrew spells:", e); }
          ...
          return (
            <div key={spell.index} className={`p-4 rounded-xl text-left transition-all duration-300 relative overflow-hidden group ${isTooHigh ? 'bg-slate-900/40 border border-slate-800' : 'bg-slate-800/80 border border-slate-600 hover:border-fuchsia-500/50 hover:shadow-[0_4px_20px_rgba(217,70,239,0.15)] hover:-translate-y-0.5'}`}>
              ...
              <div className="flex justify-between items-start mb-3 relative z-10">
                <div className="pr-4">
                  <span className={`font-black text-lg block mb-1 ${isTooHigh ? 'text-slate-500' : 'text-fuchsia-300 drop-shadow-sm'}`}>
                    {spell.name}
                    {spell.isCustom && <span className="text-[9px] ml-2 text-fuchsia-400 uppercase tracking-widest border border-fuchsia-500/30 px-1 rounded bg-fuchsia-900/20">Custom</span>}
                  </span>
