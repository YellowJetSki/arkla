...
export default function DMEditSheet({ char, charId, onCancel }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999999] flex items-center justify-center p-2 sm:p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-200">
      
      <div className="bg-slate-900 border border-amber-500/50 rounded-2xl w-full max-w-4xl shadow-[0_0_50px_rgba(245,158,11,0.15)] flex flex-col max-h-[95dvh] relative overflow-hidden animate-in zoom-in-95">
        
        <div className="flex justify-between items-center bg-slate-900/80 p-4 border-b border-slate-800 shrink-0">
          ...
          <button onClick={onCancel} className="text-slate-400 hover:text-white transition-colors bg-slate-800 p-2 rounded-xl border border-slate-700">
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            {resources.length === 0 ? (
              ...
              <div className="space-y-3">
                {resources.map((res, idx) => (
                    <div key={idx} className="flex flex-wrap sm:flex-nowrap gap-3 items-end bg-slate-900 p-3 rounded-lg border border-slate-700">
        ...
        <div className="p-4 bg-slate-900/90 border-t border-slate-800 shrink-0 flex justify-end items-center gap-4">
          {isConfirming ? (
            <>
              <span className="text-amber-400 font-bold text-sm hidden sm:inline">Apply permanent changes to database?</span>
