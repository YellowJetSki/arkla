...
import { Swords, Trash2, ArrowDown, Play, Users, X, RotateCcw } from 'lucide-react';
...
export default function InitiativeTracker({ unlockedCharacters, activeEnemies, isBattleMode, onLaunchBattle, onExitBattle }) {
  ...
  // Blacklist for ghost IDs to prevent infinite loops
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(initRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setActiveTurn(data.activeTurn || 0);
        setRound(data.round || 1);
      }
      setIsLoaded(true); // Confirms we have the real Firebase state
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    const autoSync = async () => {
      ...
      // Strip out ghost IDs that we already know don't exist in the database
      const safePlayers = (unlockedCharacters || []).filter(id => id && !missingIdsTracker.current.has(id));
      const safeEnemies = (activeEnemies || []).filter(e => e && e.id && !missingIdsTracker.current.has(e.id)).map(e => e.id);
      
      const targetIds = [...safePlayers, ...safeEnemies];
      
      const hasMissing = targetIds.some(id => !currentIds.includes(id));
      const hasStale = currentIds.some(id => !targetIds.includes(id));

      if (hasMissing || hasStale) {
        let newOrder = [...initiative];
        
        newOrder = newOrder.filter(item => targetIds.includes(item.id));

        if (hasMissing) {
          const missingPlayerIds = safePlayers.filter(id => !currentIds.includes(id));
          
          if (missingPlayerIds.length > 0) {
            const playerDocs = await Promise.all(missingPlayerIds.map(id => getDoc(doc(db, 'characters', id))));
            ...
            playerDocs.forEach((d, index) => {
              if (d.exists()) {
                ...
                // THE FIX: Add the ghost ID to the blacklist so we never try again!
  ...
  const saveInitiative = async (newOrder, newTurn, newRound) => {
    ...
    const activeActor = newOrder[newTurn];
    ...
    <div className="bg-slate-900 border border-slate-700 rounded-2xl p-4 shadow-lg h-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400">Round {round} • Turn {activeTurn + 1}</p>
        </div>
      ...
      {initiative.length > 0 && (
        <button onClick={nextTurn} className="w-full mt-4 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-white font-black py-2.5 rounded-xl transition-all flex justify-center items-center gap-2 text-sm shadow-md">
          Next Turn <ArrowDown className="w-4 h-4" />
        </button>
      )}
    </div>
  );
