...
import { PenTool, X, Sparkles, DownloadCloud, PowerOff, UploadCloud, Star, Book, Package, Image as ImageIcon, ShieldAlert, Trash2, Map, Users, Swords, Skull, Flame, Settings } from 'lucide-react';
...
export default function DMDashboard({ onLogout }) {
  ...
  useEffect(() => { 
    window.unlockedCharactersCache = unlockedCharacters; 
  }, [unlockedCharacters]);
  ...
  const selectAllEnemies = () => {
    setSelectedEnemies(selectedEnemies.length === activeEnemies.length ? [] : activeEnemies.map(e => e.id));
  };
    ...
    <div className="flex flex-col h-[100dvh] bg-slate-950 overflow-hidden text-slate-300 font-sans relative">
      ...
      {showScratchpad && (
        <div className="fixed bottom-24 lg:bottom-6 right-4 lg:right-6 w-80 h-80 bg-amber-50 rounded-xl shadow-[8px_8px_0px_rgba(0,0,0,1)] z-[9999] border-[3px] border-slate-950 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in">
          <div className="bg-amber-400 px-4 py-3 flex justify-between items-center border-b-[3px] border-slate-950 shrink-0">
            <span className="text-slate-950 font-black text-xs flex items-center gap-2 tracking-widest uppercase">
              <PenTool className="w-4 h-4"/> DM Scratchpad
            </span>
            <button onClick={() => setShowScratchpad(false)} className="text-slate-950 hover:bg-amber-300 p-1 rounded transition-colors">
          ...
          <div className="flex-1 relative">
            <DebouncedTextarea 
              ...
              placeholder="Jot down quick notes, hidden HP..." 
              className="w-full h-full p-4 bg-transparent text-amber-950 text-sm focus:outline-none resize-none font-bold custom-scrollbar leading-relaxed" 
            />
          </div>
        </div>
      )}

      {/* Sleek De-cluttered Header */}
      <header className="h-14 bg-slate-900 border-b-[3px] border-slate-950 flex items-center justify-between px-4 shrink-0 z-40 relative shadow-[0_4px_0px_rgba(0,0,0,0.5)]">
        ...
        <div className="hidden lg:flex items-center gap-1.5 flex-1 justify-center px-4">
          <button onClick={() => setActiveManager('rules')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
            <Book className="w-3.5 h-3.5"/> Rules
          </button>
          <button onClick={() => setActiveManager('items')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
            <Package className="w-3.5 h-3.5"/> Items
          </button>
          <button onClick={() => setActiveManager('spells')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
            <Flame className="w-3.5 h-3.5"/> Spells
          </button>
          <button onClick={() => setActiveManager('feats')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
            <Sparkles className="w-3.5 h-3.5"/> Feats
          </button>
          <button onClick={() => setActiveManager('handouts')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
            <ImageIcon className="w-3.5 h-3.5"/> Handouts
          </button>
          <div className="w-px h-4 bg-slate-700 mx-1"></div>
          <button onClick={() => setActiveManager('xp')} className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-amber-500 hover:text-amber-400 bg-slate-950 hover:bg-slate-800 px-3 py-2 rounded-lg border border-slate-800 transition-colors shadow-inner">
        ...
        <div className="flex items-center gap-3 shrink-0 relative">
          <button 
            ...
            className="text-amber-500 hover:text-amber-400 hover:bg-slate-800 p-1.5 rounded transition-colors" 
            title="Scratchpad"
          >
          ...
          <button 
            ...
            className={`text-slate-400 flex items-center gap-1 hover:text-white ${showSessionMenu ? 'bg-slate-800' : ''} p-1.5 rounded transition-colors`} 
            title="Session Controls"
          >
            <Settings className="w-4 h-4"/>
          </button>
          
          {/* Condensed Session Dropdown */}
          {showSessionMenu && (
            <div className="absolute top-12 right-12 w-48 bg-slate-900 border-[3px] border-slate-950 rounded-xl shadow-[6px_6px_0px_rgba(0,0,0,1)] flex flex-col p-2 z-[9999] animate-in fade-in zoom-in-95">
               <input 
                 type="file" 
                 ref={fileInputRef} 
                 className="hidden" 
                 accept=".json" 
                 onChange={(e) => { handleImportCampaign(e); setShowSessionMenu(false); }} 
               />
               <button 
                 onClick={() => { fileInputRef.current.click(); }} 
                 className="flex items-center gap-2 w-full text-left px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-800 hover:text-white rounded"
               >
                 <UploadCloud className="w-4 h-4"/> Import Backup
               </button>
               <button 
                 onClick={() => { handleExportCampaign(); setShowSessionMenu(false); }} 
                 className="flex items-center gap-2 w-full text-left px-3 py-2 text-xs font-bold text-slate-300 hover:bg-slate-800 hover:text-white rounded"
               >
               ...
               <button 
                 onClick={() => { confirmClearConditions(); setShowSessionMenu(false); }} 
                 className="flex items-center gap-2 w-full text-left px-3 py-2 text-xs font-bold text-fuchsia-400 hover:bg-slate-800 hover:text-fuchsia-300 rounded"
               >
                 <Sparkles className="w-4 h-4"/> Sweep Conditions
               </button>
               <button 
                 onClick={() => { confirmResetSession(); setShowSessionMenu(false); }} 
                 className="flex items-center gap-2 w-full text-left px-3 py-2 text-xs font-bold text-red-500 hover:bg-slate-800 hover:text-red-400 rounded"
               >
                 <Trash2 className="w-4 h-4"/> Wipe Map & Enemies
               </button>
            </div>
          )}
          ...
          <button 
            onClick={onLogout} 
            className="text-slate-500 hover:text-white hover:bg-slate-800 p-1.5 rounded transition-colors ml-1" 
            title="Logout"
          >
        ...
        <div className={`${mobileTab === 'initiative' ? 'flex' : 'hidden'} lg:flex bg-slate-950 flex-col p-3 md:p-4 overflow-hidden lg:border-r-2 border-slate-900 h-full w-full`}>
          <div className="bg-slate-900 border-[3px] border-slate-950 rounded-xl p-3 shadow-[4px_4px_0px_rgba(0,0,0,1)] flex flex-row items-center justify-between relative overflow-hidden mb-3 shrink-0">
            ...
            <div className="flex items-center gap-3 relative z-10 pl-2">
                ...
                <h2 className="text-sm font-black text-white uppercase tracking-widest drop-shadow-[1px_1px_0px_rgba(0,0,0,1)] hidden sm:block">
                  War Table
                </h2>
            </div>
            <button 
      ...
      <div className="lg:hidden fixed bottom-0 left-0 w-full z-40 bg-slate-950 border-t-[3px] border-slate-900 shadow-[0_-4px_20px_rgba(0,0,0,0.8)] pb-safe">
        <div className="flex overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] p-2 gap-2 bg-slate-900/90 backdrop-blur-md snap-x">
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Party</span>
          </button>
          ...
          >
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">Threats</span>
          </button>
          
          <div className="w-0.5 h-8 bg-slate-800 mx-1 shrink-0 self-center"></div>
          
          <button 
            onClick={() => setActiveManager('rules')} 
            className="snap-center shrink-0 min-w-[70px] flex flex-col items-center justify-center gap-1 p-2 rounded-xl transition-all border-2 bg-slate-950 text-slate-400 border-slate-900 hover:text-white shadow-inner"
          >
          ...
          <button 
            onClick={() => setActiveManager('items')} 
            className="snap-center shrink-0 min-w-[70px] flex flex-col items-center justify-center gap-1 p-2 rounded-xl transition-all border-2 bg-slate-950 text-slate-400 border-slate-900 hover:text-white shadow-inner"
          >
          ...
          <button 
            onClick={() => setActiveManager('spells')} 
            className="snap-center shrink-0 min-w-[70px] flex flex-col items-center justify-center gap-1 p-2 rounded-xl transition-all border-2 bg-slate-950 text-slate-400 border-slate-900 hover:text-white shadow-inner"
          >
            <Flame className="w-4 h-4"/> 
            <span className="text-[9px] font-black uppercase tracking-widest">Spells</span>
          </button>
          
          <button 
            onClick={() => setActiveManager('feats')} 
            className="snap-center shrink-0 min-w-[70px] flex flex-col items-center justify-center gap-1 p-2 rounded-xl transition-all border-2 bg-slate-950 text-slate-400 border-slate-900 hover:text-white shadow-inner"
          >
            <Sparkles className="w-4 h-4"/> 
            <span className="text-[9px] font-black uppercase tracking-widest">Feats</span>
          </button>
        </div>
      </div>
    </div>
