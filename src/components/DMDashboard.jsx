...
import { PenTool, X, Sparkles, DownloadCloud, PowerOff, UploadCloud, Star, Book, Package, Image as ImageIcon, ShieldAlert, Trash2, Map, Users, Swords, Skull, Flame, Loader2 } from 'lucide-react';
...
export default function DMDashboard({ onLogout }) {
  ...
  // NEW: Safety lock to prevent Initiative Tracker from auto-cleaning enemies before they download
  ...
  useEffect(() => {
    let charsDone = false;
    let enemiesDone = false;

    const checkDone = () => { if (charsDone && enemiesDone) setDataLoaded(true); };
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-slate-950 overflow-hidden text-slate-300 font-sans relative">
      ...
      <main className="flex-1 flex flex-col lg:grid lg:grid-cols-3 min-h-0 z-10 bg-slate-950 pb-[76px] lg:pb-0">
        ...
        {/* SAFETY RENDER: Only render the grid once Firebase confirms data is loaded */}
        {!dataLoaded ? (
          <div className="col-span-3 flex flex-col items-center justify-center h-full space-y-4">
            <Loader2 className="w-12 h-12 text-red-500 animate-spin" />
            <span className="text-slate-500 font-black uppercase tracking-widest text-sm">Syncing Campaign...</span>
          </div>
        ) : (
          <>
