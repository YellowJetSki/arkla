...
export default function EnemyForge({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!enemy.name) {
      setDialog({ isOpen: true, title: 'Missing Name', message: 'Your monster requires a name to be cataloged.', type: 'alert' });
      return;
    }
    ...
    const formattedEnemy = {
      ...
      speed: typeof enemy.speed === 'string' ? { walk: enemy.speed } : enemy.speed,
      ...
      isHomebrew: true,
      index: `hb_monster_${Date.now()}` // Using 'index' to match API conventions
    };

    try {
      await setDoc(doc(db, 'homebrew_enemies', formattedEnemy.index), formattedEnemy);
      setDialog({
        isOpen: true,
        title: 'Monster Forged!',
        message: `${enemy.name} is now permanently recorded in the Bestiary. You can search for and spawn it from the Encounter Builder.`,
        type: 'alert',
        onConfirm: () => {
          closeDialog();
          onClose();
      ...
      console.error("Error saving to Bestiary:", error);
      setDialog({ isOpen: true, title: 'Error', message: 'Failed to scribe monster into the database.', type: 'alert' });
      setIsSaving(false);
    }
  };
        ...
        <div className="bg-slate-900/90 backdrop-blur-2xl border border-red-500/40 rounded-3xl w-full max-w-4xl shadow-[0_0_60px_rgba(220,38,38,0.2)] flex flex-col max-h-[90dvh] relative overflow-hidden animate-in zoom-in-95 duration-500">
            ...
            {activeTab === 'actions' && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-2">
                ...
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 shadow-inner flex flex-col h-full">
                  ...
                  <div className="bg-slate-900/80 p-4 rounded-xl border border-amber-900/50 shadow-inner mt-auto">
                    ...
                    <button type="button" onClick={addTrait} className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold text-xs uppercase tracking-wider py-2 rounded-lg flex items-center justify-center gap-1.5 transition-colors shadow-sm"><Plus className="w-3 h-3" /> Add Trait</button>
                  </div>
                </div>
                ...
                <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-5 shadow-inner flex flex-col h-full">
                  ...
                  <div className="bg-slate-900/80 p-4 rounded-xl border border-red-900/50 shadow-inner mt-auto">
                    ...
                    <button type="button" onClick={addAction} className="w-full bg-red-600 hover:bg-red-500 text-white font-bold text-xs uppercase tracking-wider py-2 rounded-lg flex items-center justify-center gap-1.5 transition-colors shadow-sm"><Plus className="w-3 h-3" /> Add Action</button>
                  </div>
                </div>
          ...
          <div className="p-4 bg-slate-900/90 border-t border-slate-800 shrink-0 relative z-10 flex justify-end">
            <button 
              ...
              disabled={isSaving}
              className="w-full sm:w-auto px-8 py-3.5 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white font-black uppercase tracking-widest text-sm rounded-xl transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:shadow-[0_0_30px_rgba(220,38,38,0.5)] flex items-center justify-center gap-2"
            >
              {isSaving ? 'Scribing to Archives...' : <><Save className="w-5 h-5" /> Save to Bestiary</>}
            </button>
          </div>
