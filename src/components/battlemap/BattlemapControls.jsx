...
export default function BattlemapControls({
  ...
  return (
    <div className="bg-slate-900/80 backdrop-blur-md border-b border-indigo-500/50 p-3 shadow-[0_0_30px_rgba(99,102,241,0.15)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0 z-20">
      ...
      <div className="flex flex-nowrap items-center gap-2 w-full md:w-auto overflow-x-auto custom-scrollbar pb-1">
        <div className="flex items-center bg-slate-950/80 p-1.5 rounded-xl border border-slate-700/80 shadow-inner mr-2 shrink-0">
          ...
          {isDrawingMode && (
            <div className="flex items-center gap-1.5 px-2 border-l border-slate-700/50 ml-1 pl-2 shrink-0">
              ...
              {['#ef4444', '#3b82f6', '#10b981', '#f59e0b', '#ffffff', '#000000'].map(c => (
                <button key={c} onClick={() => setDrawingColor(c)} className={`w-5 h-5 rounded-full border-2 transition-all shrink-0 ${drawingColor === c ? 'scale-110 border-white shadow-md' : 'border-transparent opacity-60 hover:opacity-100'}`} style={{ backgroundColor: c }} />
              ))}
            </div>
          )}
          <button onClick={handleClearDrawings} className="text-slate-500 hover:text-red-400 p-2 ml-1 border-l border-slate-700/50 transition-colors shrink-0"><Eraser className="w-4 h-4" /></button>
        </div>
