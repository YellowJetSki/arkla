...
// Extracts names in quotes, e.g., Brog "The Smasher" -> "The Smasher"
...
export default function DMBattleMap() {
  ...
  // FIXED: Proper dynamic subscription pipeline to characters and session
  useEffect(() => {
    let unsubChars = () => {};
    ...
    const unsubSession = onSnapshot(doc(db, 'campaign', 'main_session'), (sessionSnap) => {
      if (sessionSnap.exists()) {
        ...
        unsubChars(); // Clean up old subscription
    ...
    <div className="flex-1 flex flex-col bg-slate-950 min-h-0 relative h-full">
      ...
      <div className="bg-slate-900 border-[3px] border-slate-950 rounded-2xl mb-4 p-3 shadow-[6px_6px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0 z-20">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto overflow-x-auto custom-scrollbar pb-1 md:pb-0">
          ...
          <div className="flex items-center bg-slate-950 p-1.5 rounded-xl border-2 border-slate-900 shadow-inner shrink-0 mr-1">
