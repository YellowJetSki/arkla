        ...
        <div className="fixed inset-0 z-[999999] pointer-events-none flex items-center justify-center bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-150">
          <div className="relative flex flex-col items-center animate-in zoom-in-90 duration-200">
            <img 
              ...
              className={`max-w-[400px] max-h-[400px] object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.8)] ${tokens[hoveredTokenId].hp <= 0 ? 'grayscale opacity-80' : ''}`} 
            />
            <h2 className={`mt-6 text-4xl font-black tracking-widest uppercase drop-shadow-xl ${tokens[hoveredTokenId].hp <= 0 ? 'text-red-500 line-through' : 'text-white'}`}>
        ...
        return (
          ...
          >
            <div className="relative w-full h-full p-0.5 group/token flex items-center justify-center">
               ...
               {isSelected && !isDead && <div className={`absolute inset-0 rounded-full animate-ping opacity-50 ${token.type === 'player' ? 'bg-indigo-500' : 'bg-red-500'}`}></div>}
               
               {/* THE MINIATURE OVERHANG BASE */}
               <div className={`absolute bottom-0 left-0 right-0 h-full rounded-full border-2 transition-colors duration-500 bg-slate-900/40 backdrop-blur-sm 
                 ${isDead ? 'border-red-900/50 shadow-none bg-red-950/40' : 
                   isSelected ? (token.type === 'player' ? 'border-indigo-400 shadow-[0_0_20px_rgba(99,102,241,0.6)]' : 'border-red-400 shadow-[0_0_20px_rgba(239,68,68,0.6)]') 
                   : (token.type === 'player' ? 'border-indigo-600 shadow-[0_10px_15px_rgba(0,0,0,0.8)]' : 'border-red-900 shadow-[0_10px_15px_rgba(0,0,0,0.8)]')}`}>
               </div>
               ...
               <img 
                 src={token.img} 
                 alt={token.name} 
                 className={`absolute bottom-[10%] left-1/2 -translate-x-1/2 w-[140%] h-[140%] object-contain drop-shadow-[0_5px_10px_rgba(0,0,0,0.7)] pointer-events-none transition-all duration-500
                   ${token.isHidden ? 'opacity-30' : ''} 
                   ${isDead ? 'grayscale opacity-50 translate-y-4 scale-y-75 mix-blend-luminosity' : ''}`} 
                 draggable={false} 
               />
               ...
               {isDead && (
                 <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-50 animate-in zoom-in duration-300">
                    <span className="text-red-600 font-black text-4xl drop-shadow-[0_2px_2px_rgba(0,0,0,1)] opacity-70">×</span>
                 </div>
               )}
               ...
               {/* Status Indicators */}
               {!isDead && (token.conditions?.length > 0 || token.isConcentrating) && (
