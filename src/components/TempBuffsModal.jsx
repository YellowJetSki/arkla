...
export default function TempBuffsModal({ charId, tempBuffs, isDM, activeTheme, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-md shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col relative overflow-hidden animate-in zoom-in-95">
        
        <div className={`p-4 border-b border-slate-800 flex justify-between items-center bg-slate-900/90 relative z-10`}>
          <h2 className={`text-lg font-black ${activeTheme.text} flex items-center gap-2 uppercase tracking-widest`}>
            <Sparkles className="w-5 h-5" /> Temporary Buffs
          </h2>
        ...
        <div className="p-6 space-y-6">
          {!isDM && (
            <form onSubmit={handleAddBuff} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 shadow-inner flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                ...
                <select 
                  ...
                  className="w-20 bg-slate-950 border border-slate-600 rounded-lg px-2 py-2.5 text-white text-sm focus:outline-none"
                >
              ...
              <button disabled={isSaving} type="submit" className={`w-full py-3 rounded-lg font-black text-white text-xs uppercase tracking-widest ${activeTheme.bg} hover:opacity-90 transition-all shadow-md`}>
          ...
          <div>
            ...
            {tempBuffs.length === 0 ? (
              <p className="text-sm text-slate-500 italic text-center p-4 bg-slate-900/50 rounded-xl border border-slate-800 border-dashed">No temporary buffs or debuffs active.</p>
            ) : (
              <div className="space-y-2 max-h-[40vh] overflow-y-auto custom-scrollbar pr-2">
                {tempBuffs.map((buff, idx) => (
                  <div key={idx} className="bg-slate-900 border border-slate-700 rounded-xl p-3 flex items-center justify-between group shadow-sm hover:border-slate-500 transition-colors">
                    ...
                    {!isDM && (
                      <button onClick={() => handleRemoveBuff(buff)} className="text-slate-500 hover:text-red-400 bg-slate-950 border border-slate-800 p-2 rounded-lg transition-colors shadow-inner">
